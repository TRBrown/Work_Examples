				  ...
				  float2 inTxr		: TEXCOORD0)
{
	...
	//Surface deformation
	//*****
	float3 vecRadius = Pos;
	float a = sqrt(Square(vecRadius.x) + Square(vecRadius.z));
	float magRadius = sqrt(Square(vecRadius.y) + Square(a));
	
	float4 normForceDir = normalize(mul(forceDir, wInvMat)); //Transform vector to Local space
	float3 hitPoint = float3(0.0f, 0.0f, 0.0f) + (-normForceDir * magRadius);
	
	float3 vecHitpointToPos = Pos - hitPoint;
	a = sqrt(Square(vecHitpointToPos.x) + Square(vecHitpointToPos.z));
	float magHitpointToPos = sqrt(Square(vecHitpointToPos.y) + Square (a));
	
	float3 displacement = (forceStrength * 
							(min((magHitpointToPos - forceRadius), 0)) 
						   		* normForceDir);
	float4 dispV4 = float4(displacement.x, displacement.y, displacement.z, 0.0f);
			   ...
			   float3 lightDirT2 : 	TEXCOORD3) : COLOR
{
	...
	//Spec Calc
	float t1 = pow(max(dot(r1, toEyeT), 0.0f), matPower);
	float t2 = pow(max(dot(r2, toEyeT), 0.0f), matPower);
	//Diff Calc
