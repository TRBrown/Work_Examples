...
CollisionResult CollisionManager::MouseOverModelCheck(int xPos, int yPos, ID3DXMesh* mesh, D3DXMATRIX* worldMatrix)
{
...
//0 = false; none 0 = distance above supplied check distance
bool CollisionManager::DistanceGreaterThan(D3DXVECTOR3 position1, D3DXVECTOR3 position2, float dist)
{
	float magnitude = vec3Magnitude(position1, position2);
	if(dist >= magnitude)
	{
		return 1;
	}
...
bool CollisionManager::DistanceGreaterThan(D3DXVECTOR4 position1, D3DXVECTOR4 position2, float dist)
{
	float magnitude = vec4Magnitude(position1, position2);
	if(dist > magnitude)
	{
		return 1;
	}
