...
D3DXVECTOR4 CustomModel::D3DXReset = D3DXVECTOR4 (1.0f, 1.0f, 1.0f, 1.0f); 
...
CustomModel::CustomModel(const CustomModel & rhs)
{
	...
	m_sNormalMapFN  =       rhs.m_sNormalMapFN;
	m_sFXFN			=		rhs.m_sFXFN;
	...
	m_mViewProjection		=		rhs.m_mViewProjection;
	m_mWorldViewProjection			=		rhs.m_mWorldViewProjection;
	m_mWorldinverse	=		rhs.m_mWorldinverse;
	...
	amb2v4			=		rhs.amb2v4;
	vec2Dir			=		rhs.vec2Dir;
	m_v4_HighLightColour		=		rhs.m_v4_HighLightColour;
	veceye			=		rhs.veceye;
...
void CustomModel::Loadfx()
{
	...
	m_pWFXFN = (wchar_t*)malloc(size * sizeof(wchar_t));
	...
	{
		const WCHAR* ErrorMsg = DXGetErrorString(hr);
		m_bError = true;
		assert(m_bError != true);
	}
...
void CustomModel::AssignParameters(D3DLIGHT9& light1, 
	                               D3DLIGHT9& light2, const D3DXVECTOR3 *eye, 
								   const D3DMATRIX *view, const D3DMATRIX *proj)
{
	m_pLight1 = &light1;
	m_pLight2 = &light2;
	m_mView = *view;
	...
	m_mWorldViewProjection = m_mWorld * m_mViewProjection;

	D3DXVECTOR4 diff1( m_pLight1->Diffuse.r, m_pLight1->Diffuse.g, m_pLight1->Diffuse.b, m_pLight1->Diffuse.a );
	D3DXVECTOR4 spec1( m_pLight1->Specular.r, m_pLight1->Specular.g, m_pLight1->Specular.b, m_pLight1->Specular.a );
	D3DXVECTOR4 amb1( m_pLight1->Ambient.r, m_pLight1->Ambient.g, m_pLight1->Ambient.b, m_pLight1->Ambient.a ); 
	D3DXVECTOR4 Pos1( m_pLight1->Position.x, m_pLight1->Position.y, m_pLight1->Position.z, 0.0f );

	D3DXVECTOR4 diff2( m_pLight2->Diffuse.r, m_pLight2->Diffuse.g, m_pLight2->Diffuse.b, m_pLight2->Diffuse.a );
	D3DXVECTOR4 spec2( m_pLight2->Specular.r, m_pLight2->Specular.g, m_pLight2->Specular.b, m_pLight2->Specular.a );
	D3DXVECTOR4 amb2( m_pLight2->Ambient.r, m_pLight2->Ambient.g, m_pLight2->Ambient.b, m_pLight2->Ambient.a ); 
	D3DXVECTOR4 Dir1( m_pLight2->Direction.x, m_pLight2->Direction.y, m_pLight2->Direction.z, 0.0f );	

	D3DXMatrixInverse(&m_mWorldinverse, 0, &m_mWorld);
	...
	m_pEffect->SetMatrix( "wvpMat", &m_mWorldViewProjection);
	m_pEffect->SetMatrix( "wInvMat", &m_mWorldinverse);
	...
	m_pEffect->SetVector("highLightColour",&m_v4_HighLightColour);
